...
#' @noRd
#'
#' @examples seal:::sys_msg_error("Data incomplete", c("x" = "Please fill in the data"), func = "data_check")
sys_msg_error = function(title, error, func = NULL, silent = FALSE, flag = TRUE){
  if(!silent){
    if(!is.null(func)){
      cli::cli_alert_danger("Error in {.fn {func}}: {title}")
    } else {
      cli::cli_alert_danger("Error: {title}")
    }
    for(i in error){
