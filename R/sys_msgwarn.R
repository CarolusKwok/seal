...
#'
#' @return a standardized text message
#' @keywords internal
#' @noRd
#'
#' @examples seal::sys_msgwarn("Data incomplete", c("!" = "Please fill in the data"), func = "data_check")
sys_msgwarn = function(title,
                       error,
                       silent = FALSE,
                       func = NULL,
                       return_flag = TRUE){
  #Check if sys_msgwarn has error ####
  if(!hasArg(title)){
    cli::cli_text(cli::bg_yellow(cli::col_black("Error in {.fn sys_msgwarn}: No {.code title} or {.code error}")))
