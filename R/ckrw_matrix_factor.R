#' Pre-check for r/w: Check `factor` matrix fulfills requirement
#'
...
#' @noRd
#'
#' @examples ckrw_factor(factor)
ckrw_factor = function(factor, silent = FALSE, func = NULL){
  ...
  #Check if all `factor`, `label`, are filled ####
  if(anyNA(dplyr::select(.data = factor, factor, label))){
    return(invisible(seal:::sys_msg_error(title = "`factor` & `label` in `factor` must be all filled.",
                                          error = "Please check `factor` and remove any blanks.")))
  }

  #Check if `abbr` is all filled or not all filled ####
  len = nrow(factor)
  len_na = sum(is.na(factor$abbr))
  if(len_na != len & len_na != 0){
    return(seal:::sys_msg_error(title = "`abbr` in `factor` must be all filled or all empty,",
                                error = "Please check `abbr` in `factor`."))
  }
